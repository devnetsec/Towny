		...
		for (Resident resident : new ArrayList<>(universe.getJailedResidentMap()))
			if (resident.hasJailTime()) {
				...
				if (resident.getJailHours() <= 1) {
					plugin.getScheduler().runLater(() -> JailUtil.unJailResident(resident, UnJailReason.SENTENCE_SERVED), 20);
					continue;
				}
				...
				if (hourlyJailFee > 0) {
					...
					if (!jailTown.getAccount().withdraw(hourlyJailFee, "Jailee Hourly Fee for " + resident.getName())) {
						...
						plugin.getScheduler().runLater(() -> JailUtil.unJailResident(resident, UnJailReason.INSUFFICIENT_FUNDS), 20);
						continue;
					} else {
